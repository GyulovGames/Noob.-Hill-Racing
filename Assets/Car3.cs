...
public class Car3 : MonoBehaviour
{
    ...
    {
        if (YandexGame.savesData.Car3_Upgrades[0] == 1)
        {
            engineForce = 160f;
        }
        else
        {
            engineForce = 160 + 11 * YandexGame.savesData.Car3_Upgrades[0];
        }

        if (YandexGame.savesData.Car3_Upgrades[2] == 1)
        {
            frontWheelsForce = 27f;
        }
        else
        {
            frontWheelsForce = 27f * YandexGame.savesData.Car3_Upgrades[2];
        }

        JointSuspension2D suspension = frontWheel_Joint.suspension;
        suspension.dampingRatio = carDampinRatio *= YandexGame.savesData.Car3_Upgrades[1];
        frontWheel_Joint.suspension = suspension;
        rearWheel_Joint.suspension = suspension;

        wheelsPhysicsMaterial.friction = wheelsGrip *= YandexGame.savesData.Car3_Upgrades[3];
        ...
        if (YandexGame.savesData.Car3_Upgrades[4] == 1)
        {
            curentFuelAmount = fuelAmount;
        }
        else
        {
            curentFuelAmount = fuelAmount + 10 * YandexGame.savesData.Car3_Upgrades[4];
        }
    }
    ...
    private void OnTriggerEnter2D(Collider2D collision)
    {
        ...
        {
            curentFuelAmount = fuelAmount * YandexGame.savesData.Car3_Upgrades[4];
            GameCanvas.Instance.UpdateFuelBarOnStart(curentFuelAmount);
        }
    }
}
